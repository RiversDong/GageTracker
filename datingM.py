...
def dating(block, gene_bed, reference, branch, outpath, agefile, voting, gene2ratio):
    ...
    out_title = "\t".join(tmp_title2) + "\t" + "\t".join(tmp_title1)
    age_out.write(out_title+"\n")
    for i in geneindex:
        iinfo = list(brHomoPd.loc[i,:]); genei=iinfo[0]
        tablei = iinfo[1:] # not include the gene in the focal species
        if sum(tablei) == 0:
            age = branch[0]
        else:
            ##first_homo = getIndex(tablei)[-1]
            ##age = branch[first_homo+1]
            homoindex = getIndex(tablei) # first homo index 
            for j in homoindex:
                jsublist = tablei[:j+1]
                # 3/11
        ...
        if genei in gene2ratio:
            ...
            mask_ratio = "NA"
        resultInfo = [confidence, age, ichr,istart,iend, genei,mask_ratio,"1"+"\t"+"\t".join(tmp_table)]
