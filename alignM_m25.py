...
def addPrefix(refDic, newFaPath, process_num=1, mask="false"):
    ...
    tmp_ref_fasta = list(refDic.values())
    ref_fasta = []
    for i in tmp_ref_fasta:
        ref_fasta.extend(i)

    # 使用 ThreadPoolExecutor 来执行多线程操作
    with ThreadPoolExecutor(max_workers=process_num) as executor:
        ...
        for i in ref_fasta:
            ...
            # --- following is the newly added for masked the reference genome ---
            masked_ibase = os.path.join(newFaPath, ibase + ".msk")
            cmd_tantan = "tantan {0} > {1}".format(i, masked_ibase)
            futures.append(executor.submit(os.system, cmd_tantan))
            # ---end ---
    ...
    if mask == "true":
        ...
        for i in new_reference:
            ...
            for j in refs:
                window_tmp = j+".tmp"
                window_mask=j+".w"
                cmd1 = "windowmasker -mk_counts -in {0} -infmt fasta -out {1} -sformat obinary ".format(j, window_tmp); cmd1_list.append(cmd1)
...
def repeatMasking(tfa, masking):
    ...
    with open(os.devnull, 'w') as devnull:
        subprocess.run(cmd1, check=True, stdout=devnull, stderr=devnull)
